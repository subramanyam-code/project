      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">{total} department{total !== 1 ? 's' : ''}</p>
          </div>
          {selectedCompany && (
        ...
        <Card>
          {!selectedCompany ? (
            <div className="py-16 text-center text-gray-400">
              ...
              {companies.length === 0 && (
                <p className="text-sm mt-1">No companies exist yet. <Link href="/admin/companies" className="text-blue-600 hover:underline">Create one first.</Link></p>
              )}
            </div>
          ) : (
                    ...
                    <tr key={d.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-gray-900">{d.department_name}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <button onClick={() => openEdit(d)} className="text-xs text-blue-600 hover:text-blue-800 font-medium px-2 py-1 rounded hover:bg-blue-50">Edit</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 z-10">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-lg font-semibold text-gray-900">{modal === 'create' ? 'New Department' : 'Edit Department'}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input value={form.department_name} onChange={e => { setForm(f => ({ ...f, department_name: e.target.value })); setErrors(er => ({ ...er, department_name: undefined })); }}
                  className={`w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.department_name ? 'border-red-400' : 'border-gray-300'}`}
                  placeholder="e.g. Engineering" />
                {errors.department_name && <p className="text-xs text-red-600 mt-1">{errors.department_name}</p>}
              </div>
              <div>
                ...
                <select value={form.company_id} onChange={e => { setForm(f => ({ ...f, company_id: e.target.value })); setErrors(er => ({ ...er, company_id: undefined })); }}
                  className={`w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.company_id ? 'border-red-400' : 'border-gray-300'}`}>
                  <option value="">— Select Company —</option>
                  {companies.map(c => <option key={c.id} value={c.id}>{c.company_name}</option>)}
                </select>
