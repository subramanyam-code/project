...
export default function NotificationsPage() {
  ...
  const getTypeColor = (notif: Notification) => {
    ...
    if (t === 'success') return 'bg-green-100 text-green-800';
    if (t === 'warning') return 'bg-yellow-100 text-yellow-800';
    if (t === 'error') return 'bg-red-100 text-red-800';
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {notifications.map((notification) => (
              <Card
                key={notification.id}
                className={`p-4 ${!notification.is_read ? 'bg-blue-50 border-l-4 border-blue-500' : 'bg-white'}`}
              >
