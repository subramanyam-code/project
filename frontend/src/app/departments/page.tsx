  ...
  const columns = [
    ...
    { key: 'description', label: 'Description' },
    {
      ...
      render: (dept: Department) =>
        dept.company?.company_name ?? dept.company?.name ?? 'N/A',
    },
    {
      key: 'is_active',
      label: 'Status',
    ...
    },
    {
      ...
      render: (dept: Department) => (
        <div className="flex gap-2">
          {canEdit && (
            <Button variant="secondary" size="sm" onClick={() => handleEdit(dept)}>
              Edit
            </Button>
          )}
          {canDelete && (
  ...
  return (
    <ProtectedLayout requiredPermissions={['super_admin', 'company_admin']}>
      <div className="p-6">
        ...
        {isModalOpen && (
          <DepartmentModal department={editingDepartment} onClose={handleCloseModal} />
        )}
      </div>
    </ProtectedLayout>
  );
